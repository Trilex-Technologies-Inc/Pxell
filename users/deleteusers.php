...
if ($comptListMembers > 0) {
    ...
    for ($i = 0;$i < $comptListMembers;$i++) {
        ...
        echo '<strong>' . $listMembers->mem_login[$i] . '</strong> (' . $listMembers->mem_name[$i] . ')';
        echo '</div>';
    }
    echo '</div>';
}
...
echo '<h5 class="alert-heading">' . $strings["reassignment_user"] . '</h5>';
echo '<p class="mb-2">' . sprintf($strings["there"] . ' %d ' . $strings["projects"] . ' ' . $strings["owned_by"], $totalProjects) . '</p>';
echo '<p class="mb-3">' . sprintf($strings["there"] . ' %d ' . $strings["tasks"] . ' ' . $strings["owned_by"], $totalTasks) . '</p>';

echo '<hr>';

echo '<div class="form-group">';
echo '<label for="atSelect"><strong>' . $strings["reassign_to"] . ':</strong></label>';
...
echo '<select name="at" id="atSelect" class="form-control">';
